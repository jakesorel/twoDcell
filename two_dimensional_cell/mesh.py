...
class Mesh:
    def __init__(self,x,radius,L,buffer=None,tri=None,fill=True, id=None, name=None, load=None, run_options=None):
    ...
    def triangulate(self):
        self.n_c = self.x.shape[0]
        self.y, self.n_C,self.idxs = extend_domain(self.x, self.L, self.radius, self.n_c, self.corner_shifts, self.mid_shifts)
    ...
    def get_angles(self):
        self.ttheta_j, self.hm_j, self.hp_j = _ttheta(self.V_in_j, self.V_out_j, self.no_touch_j, self.radius,
                                                      self.tx,
                                                      self.vs3,
                                                      self.lrij, self.vm1, dir=1)

        self.ttheta_k, self.hm_k, self.hp_k = _ttheta(self.V_in_k, self.V_out_k, self.no_touch_k, self.radius,
                                                      self.tx,
                                                      self.vp1,
    ...
    def get_P(self):
        self.tlP, self.tlC = get_lP(self.hp_j, self.hm_j), get_lC(self.tpsi_j, self.radius)
        self.tP = self.tlP + self.tlC
        self._P = trf.assemble_tri(self.tP, self.tri)
        self.P = self._P[:self.n_c]

    def get_A(self):
        self.tAP, self.tAC = get_AP(self.hm_j, self.hp_j, self.tx), get_AC(self.tpsi_j, self.radius)
...
def _ttheta(V_in, V_out, no_touch, tR, tS, tV, nrij, vj_neighbours, dir=1):
    ...
    h_CCW, h_CW = _tintersections(tS, trf.roll3(tS, dir), tR, nrij)
...
@jit(nopython=True)
def do_classification_correction(r, radius, ttheta, hm, hp, no_touch, V_in_j, err=1e-7):
    """
    ...
    if touch_not_power_mask.any():
        ...
        d = numba_cdist(hp_circ, r) - radius ** 2
