...
def animate(x_save, L,radius, cols, n_frames=100, file_name=None, dir_name="plots", cbar=None, **kwargs):
    """
    Animate the simulation
    ...
    def animate(i):
        ...
        cbar_a = None
        if dynamic_col:
            plot_vor(ax1, x_save[skip * i], L,radius, cols[skip * i], cbar=cbar_a, **kwargs)
        else:
            plot_vor(ax1, x_save[skip * i], L,radius, cols, cbar=cbar_a, **kwargs)
        ax1.set(aspect=1, xlim=(0, L), ylim=(0, L))
