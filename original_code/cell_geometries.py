...
from differentials import _power_vertex_differentials, _circle_vertex_differentials
from tri_functions import _roll, _roll3, _tnorm, _tri_sum, _CV_matrix, _get_neighbours, \
...
class geometries:
    def __init__(self, S, R, V, tri_list, n_v, n_b):
        ...
        self.CV_matrix = _CV_matrix(self.tri_list, self.n_v, self.n_c)
        self.rj = _roll3(self.tS, 1)
        self.Rj = _roll(self.tR, 1)
        self.rk = _roll3(self.tS, -1)
        self.Rk = _roll(self.tR, -1)

        # 2. Get neighbours, and corresponding vertices
        self.neighbours, self.ls = _get_neighbours(self.tri_list)
        self.jneighbours = _roll(self.neighbours, -1)
        self.kneighbours = _roll(self.neighbours, +1)
        ...
        self.no_touch_j_mat = _repeat_mat(self.no_touch_j)
        self.no_touch_j_vec = _repeat_vec(self.no_touch_j)

        self.no_touch_k_mat = _repeat_mat(self.no_touch_k)
        self.no_touch_k_vec = _repeat_vec(self.no_touch_k)
        ...
        self.nrihp_j, self.nrihp_k, self.nrihm_j, self.nrihm_k = _tnorm(self.rihp_j), _tnorm(self.rihp_k), _tnorm(
            self.rihm_j), _tnorm(self.rihm_k)

        self.hmj_hpj = _V_Vj(self.hm_j, self.hp_j)
        self.hmk_hpk = _V_Vj(self.hm_k, self.hp_k)
        self.nhmj_hpj = _tnorm(self.hmj_hpj)
        self.nhmk_hpk = _tnorm(self.hmk_hpk)
        ...
        self.lP = _tri_sum(self.n_c, self.CV_matrix, self.tlP)
        self.lC = _tri_sum(self.n_c, self.CV_matrix, self.tlC)
        self.AP = _tri_sum(self.n_c, self.CV_matrix, self.tAP)
        self.AC = _tri_sum(self.n_c, self.CV_matrix, self.tAC)
